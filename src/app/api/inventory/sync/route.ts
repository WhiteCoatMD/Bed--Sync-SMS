  ...
  try {
    ...
    if (items.length > 0) {
      const mapped = items.map((item: Record<string, unknown>) => ({
        dealer_id,
        brand: item.brand || item.style || 'Unknown',
        model: item.model || item.display_name || item.serial_number || 'Unknown',
        series: item.category || null,
        size: mapSize(item.size as string),
        firmness: mapFirmness(item.firmness as string),
        type: mapType(item.type as string),
        price: parseFloat(String(item.cash_price || item.original_price || 0)),
        sale_price: item.sale_price ? parseFloat(String(item.sale_price)) : null,
        msrp: item.original_price ? parseFloat(String(item.original_price)) : null,
        in_stock: true,
        quantity: parseInt(String(item.quantity || 1)),
        features: Array.isArray(item.features) ? item.features : [],
        description: item.description || null,
        financing_eligible: true,
        promotion: item.sale_price ? 'On Sale' : null,
        sku: item.sku || item.serial_number || null,
